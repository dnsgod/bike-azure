...
Raw Zone → ETL → Serving Layer 구조

Snapshot 기반 시계열 모델링

Fallback 전략 포함한 운영 설계
...
Snapshot 구조 채택으로 시계열 분석 용이

Serving Layer 분리로 대시보드 조회 성능 확보

PK 설계로 중복 데이터 방지

## 4️⃣ Data Quality & 운영 설계
✔ 데이터 품질 관리

JSON Path 검증 및 매핑 오류 수정

NULL insert 방지 처리

station_name 한글 깨짐 → NVARCHAR 수정

중복 방지를 위한 복합 PK 적용

✔ 운영 안정성 설계

DB 조회 실패 시 CSV fallback 모드

Streamlit cache 적용으로 조회 성능 개선

트리거 자동 실행 → 비용 증가 우려 시 수동 전환
...
Serverless 기반 데이터 수집

Raw Zone 유지 전략

ETL Orchestration (ADF)

Snapshot 기반 SQL 모델링

운영 고려(Fallback / 비용 / 장애 대응)

까지 구현한 클라우드 데이터 파이프라인 프로젝트입니다.
